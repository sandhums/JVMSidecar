...
| **`ValueSetExpansionCache`** / **`CachedR4FhirTerminologyProvider`** | Process (Caffeine, 60s TTL) | Reuses HTS `$expand` results per terminology base. **`in` is not cached.** It calls HTS `ValueSet/$validate-code` and reads the `result` parameter by name. See [CQL `in`](#cql-in--hts-validate-code). |
...
### CQL `in` / HTS `$validate-code`

CQF `R4FhirTerminologyProvider.in` calls HTS `ValueSet/$validate-code` and casts **`Parameters[0]`** to `BooleanType`. HTS puts `code` first and `result` (`valueBoolean`) later, so the cast throws `ClassCastException`. `/v1/measure/evaluate` then reports **not-in** (`status=complete`, numerator 0). Found 2026-09-21 on CMS131 `numerator-autonomous` (`LA34398-0` from `atrius-in-loinc-answers` in `atrius-vs-autonomous-eye-exam-result`).

`CachedR4FhirTerminologyProvider.in` issues the same operation and reads the parameter named **`result`**. It does not use parameter order. A code with no system still expands. `Cms131ValueSetInTest` checks `LA34398-0` against an HTS-ordered response.
